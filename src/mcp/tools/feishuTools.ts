 ...
 */
export function registerFeishuTools(server: McpServer, feishuService: FeishuApiService | null, userAccessToken?: string, userInfo?: any): void {
  // 添加创建飞书文档工具
  server.tool(
    ...
    async ({ title, folderToken }) => {
      try {
        ...
        const newDoc = await feishuService?.createDocument(title, folderToken, userAccessToken);
        if (!newDoc) {
  ...
  server.tool(
    ...
    async ({ searchKey }) => {
      try {
        ...
        const searchResult = await feishuService.searchDocuments(searchKey, undefined, userAccessToken);
